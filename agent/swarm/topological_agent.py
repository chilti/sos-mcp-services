...
def get_topological_tools(session_id: Optional[str] = None) -> List[Any]:
    """Herramientas matemáticas y topológicas avanzadas para el modelado de variedades."""
    tools = []

    # 1. SOM Mesh Dimension Calculator via SVD (Regla espectral knoMap)
    @tool
    def calculate_som_optimal_grid(n_samples: int, ratio_principal_components: float = 1.5) -> str:
        ...
        if session_id:
            episodic_memory.record_evidence(
                ...
                claim_text=f"Malla SOM SVD: {rows}x{cols} ({rows*cols} nodos para N={n_samples})",
                evidence_source="som_svd_solver",
                evidence_payload=res
            )
        return json.dumps(res, ensure_ascii=False)
    tools.append(calculate_som_optimal_grid)
        ...
        try:
            ...
            if session_id:
                episodic_memory.record_evidence(
                    ...
                    claim_text=f"Partición Louvain: Modularidad Q = {round(float(modularity), 4)} en {len(communities)} clusters",
                    evidence_source="networkx_louvain",
                    evidence_payload=result
                )
            return json.dumps(result, ensure_ascii=False)
        except Exception as e:
            return f"Error en cálculo Louvain: {str(e)}"
    tools.append(compute_network_modularity_louvain)
...
class TopologicalAgent(BaseSpecialistAgent):
    ...
    def __init__(self, session_id: Optional[str] = None, **kwargs):
        tools = get_topological_tools(session_id=session_id)
